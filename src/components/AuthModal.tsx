...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess, language }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message : 'Erro ao iniciar sessão com Google.');
    } finally {
      setLoading(false);
    }
  };
          ...
          {tab === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="relative my-3">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-white px-2 text-slate-400 font-medium">
                    {language === 'pt' ? 'ou entrar com' : 'or continue with'}
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-2.5 px-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs sm:text-sm shadow-2xs transition-all flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-50"
              >
                ...
                <span>{language === 'pt' ? 'Continuar com Conta Google' : 'Continue with Google'}</span>
              </button>
            </form>
          )}
