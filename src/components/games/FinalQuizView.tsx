    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 animate-in fade-in">
      ...
      <div className="rounded-[2rem] bg-indigo-950 text-white p-6 sm:p-8 md:p-10 shadow-xl mb-8 relative overflow-hidden">
        <div className="relative z-10 space-y-3">
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-widest text-indigo-300">
            ...
            {hasFirstAttempt ? (
              ...
              <span className="inline-flex items-center gap-1 bg-emerald-500/20 border border-emerald-400/40 text-emerald-200 px-2.5 py-1 rounded-full">
                ...
                <span>{language === 'pt' ? '1.ª Tentativa Oficial (100 XP)' : '1st Official Attempt (100 XP)'}</span>
              </span>
            )}
          </div>
          ...
          {/* Explicit reminder about first attempt scoring rule */}
          <div className="pt-2">
            {hasFirstAttempt ? (
              <div className="inline-flex flex-wrap items-center gap-2 bg-indigo-900/60 border border-indigo-700/60 rounded-xl px-3.5 py-2 text-xs text-indigo-200">
                <span className="font-semibold text-amber-300">
                  {language === 'pt' ? 'Pontuação Oficial (1.ª tentativa):' : 'Official Score (1st attempt):'}
                </span>
                <span className="font-black text-white bg-indigo-800/90 px-2 py-0.5 rounded-md">
                  {officialFirstScore} / 100 XP
                </span>
                <span className="text-indigo-300/80">
                  {language === 'pt'
                    ? '• As tentativas seguintes são para prática e não alteram a nota oficial.'
                    : '• Further attempts are for practice and do not alter the official score.'}
                </span>
              </div>
            ) : (
              <div className="inline-flex items-center gap-2 bg-emerald-950/40 border border-emerald-500/30 rounded-xl px-3.5 py-2 text-xs text-emerald-200">
                ...
                <span>
                  {language === 'pt'
                    ? 'Nota importante: A pontuação obtida agora fica registada como a tua nota oficial permanente deste Quiz de Aprendizagem.'
                    : 'Important note: The score obtained now will be registered permanently as your official score for this Learning Quiz.'}
                </span>
              </div>
            )}
          </div>
      ...
      {submitted && (
        <div className="rounded-[2rem] bg-white border-2 border-indigo-100 p-6 sm:p-8 shadow-md mb-8 text-center space-y-4 animate-in zoom-in-95">
          <div className="text-5xl">{result.percentage >= 80 ? '🌟' : '📚'}</div>
          ...
          {hasFirstAttempt ? (
            <div className="space-y-2">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">
                {result.percentage} / 100 ({result.score} de {result.maxScore} Corretas)
              </h2>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 max-w-lg mx-auto text-xs sm:text-sm text-amber-950 space-y-1">
                <p className="font-bold flex items-center justify-center gap-1 text-amber-900">
                  ...
                  <span>
                    {language === 'pt'
                      ? `Pontuação Oficial Registada (1.ª tentativa): ${officialFirstScore} / 100 XP`
                      : `Official Registered Score (1st attempt): ${officialFirstScore} / 100 XP`}
                  </span>
                </p>
                <p className="text-amber-800/90 leading-relaxed text-[11px] sm:text-xs">
                  {language === 'pt'
                    ? 'Esta tentativa serviu para treino e aprendizagem. As tentativas seguintes não alteram a tua nota oficial da 1.ª tentativa.'
                    : 'This attempt served for learning and practice. Subsequent attempts do not modify your official score from the 1st attempt.'}
                </p>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">
                {result.percentage} / 100 XP ({result.score} de {result.maxScore} Corretas)
              </h2>
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 max-w-lg mx-auto text-xs sm:text-sm text-emerald-950 space-y-1">
                <p className="font-bold flex items-center justify-center gap-1 text-emerald-900">
                  ...
                  <span>
                    {language === 'pt'
                      ? `Pontuação Oficial Registada: ${result.percentage} / 100 XP`
                      : `Official Registered Score: ${result.percentage} / 100 XP`}
                  </span>
                </p>
                <p className="text-emerald-800/90 leading-relaxed text-[11px] sm:text-xs">
                  {language === 'pt'
                    ? 'A pontuação desta 1.ª tentativa ficou guardada permanentemente como a tua nota oficial. Podes repetir este quiz quantas vezes quiseres para continuar a treinar!'
                    : 'The score of this 1st attempt is permanently saved as your official score. You may repeat this quiz as many times as you like to practice!'}
                </p>
              </div>
            </div>
          )}
